...
const ResetPassword: React.FC = () => {
  ...
  useEffect(() => {
    if (!token) {
      setTokenError('Token no válido o expirado. Solicita uno nuevo.');
    }
  }, [token]);
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {tokenError ? (
            <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-6 rounded-lg text-center">
              <AlertTriangle size={48} className="mx-auto mb-3 text-red-600" />
              <p className="font-medium">Enlace no válido</p>
              <p className="text-sm mt-2">{tokenError}</p>
              <button
